...
params_common = {"limit": 100, "order": "asc", "sort": "ticker"}

for s in symbols:
    params = dict(params_common)
    # use underlying_ticker param where applicable
    # For futures-style tickers (CL=F) try passing as underlying_ticker first
    ...
    fname = os.path.join(SAMPLES_DIR, f"massive_contracts_{s.replace('=','_')}.json")
    with open(fname, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2)
    print(f"Saved sample for {s} -> {fname}")
    time.sleep(1)
