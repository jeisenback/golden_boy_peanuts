...
# Crude-linked instruments for which OVX is a reasonable implied-vol proxy
# (weak for XOM/CVX, so they are excluded). Issue #212.
_OVX_PROXY_INSTRUMENTS: frozenset[str] = frozenset({"CL=F", "BZ=F", "USO"})
_OVX_FIXTURE: str = "ovx_event_windows.csv"
_CLOSE_FIXTURES: dict[str, str] = {"CL=F": "cl_close_event_windows.csv"}
...
def _load_ovx_context(
    fixtures_dir: Path | None = None,
) -> tuple[dict[date, float], dict[str, dict[date, float]]]:
    """
    Load OVX and underlying closes from fixtures for the OVX implied-vol fallback.

    Returns ({}, {}) when the fixture files are absent, so replay is unaffected.
    """
    base = fixtures_dir if fixtures_dir is not None else FIXTURES_DIR
    ovx_path = base / _OVX_FIXTURE
    if not ovx_path.exists():
        return {}, {}
    ovx = load_ovx(date.min, date.max, csv_path=ovx_path)
    closes = {
        instrument: load_closes_csv(base / name)
        for instrument, name in _CLOSE_FIXTURES.items()
        if (base / name).exists()
    }
    return ovx, closes
...
def _build_feature_set(
    ...
    ovx: dict[date, float] | None = None,
    closes_by_instrument: dict[str, dict[date, float]] | None = None,
) -> FeatureSet:
    """
    ...
    Args:
        ...
        ovx: OVX index points by date. When given, crude-proxy instruments with
            no option implied vol get a gap from OVX and realized vol (#212).
        closes_by_instrument: Underlying closes used for that realized vol.
    ...
    if ovx and closes_by_instrument:
        as_of = market_state.snapshot_time.date()
        priced = {p.instrument for p in market_state.prices}
        for instrument in sorted((_OVX_PROXY_INSTRUMENTS & priced) - iv_by_instrument.keys()):
            closes = closes_by_instrument.get(instrument)
            if closes is None:
                continue
            try:
                vol_gaps.append(
                    historical_volatility_gap(instrument, as_of, closes=closes, ovx=ovx)
                )
            except ValueError as exc:
                logger.warning("OVX fallback unavailable for %s: %s", instrument, exc)
...
def replay_pipeline(
    ...
    ovx, closes_by_instrument = _load_ovx_context()
    feature_set = _build_feature_set(market_state, ovx, closes_by_instrument)
    snapshot_time = market_state.snapshot_time
